...
final class KarstCaveCarver {
    ...
    /** @param baseY lowest Y of the terrain; the bedrock sits on it */
    void carve(int chunkX, int chunkZ, ChunkData chunk, int baseY) {
        ...
        for (int x=0;x<16;x++) for (int z=0;z<16;z++) {
            ...
            int y=Math.max(baseY+8, Math.min(surface-8, baseY+16+(int)((h>>>8)&31)));
            for(int dx=-2;dx<=2;dx++) for(int dz=-2;dz<=2;dz++) {
                ...
                for(int dy=-2;dy<=2;dy++) if(dx*dx+dy*dy+dz*dz<=4) {
                    int ly=y+dy; if(ly>baseY+1&&ly<neighbourSurface) chunk.setBlock(lx,ly,lz,Material.AIR);
                }
            }
        }
    }
