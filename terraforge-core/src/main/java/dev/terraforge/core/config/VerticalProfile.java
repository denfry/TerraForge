...
public record VerticalProfile(int seaLevel, int minY, int maxY, double metersPerBlock) {
    ...
    public TerraForgeConfig.TerrainSection applyTo(TerraForgeConfig.TerrainSection defaults) {
        return new TerraForgeConfig.TerrainSection(seaLevel, minY, maxY, defaults.verticalExaggeration(), metersPerBlock, defaults.fallbackElevation(), defaults.bedrockThickness(), defaults.smoothing(), defaults.reliefCurveMeters(), null, null);
    }
    /**
